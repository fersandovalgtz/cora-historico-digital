...
def add_taxonomy(encoding_desc: ET.Element) -> None:
    ...
    taxonomy = ET.SubElement(
        class_decl,
        q("taxonomy"),
        {xml_attr("id"): "chdMachineAuthority"},
    )
    categories = (
...
def add_header(root: ET.Element, accepted_count: int, residual_count: int) -> None:
    ...
    text_element(
        extent,
        "measure",
        f"{accepted_count} machine-accepted lexical entries",
        unit="entry",
        quantity=str(accepted_count),
    )
    text_element(
        extent,
        "measure",
        f"{residual_count} residual machine candidates",
        unit="candidate",
        quantity=str(residual_count),
    )
    ...
    text_element(
        availability,
        "licence",
        "Original project metadata and derived annotations: CC BY 4.0. Historical source: public domain.",
    ...
    bibl_struct = ET.SubElement(
        list_bibl,
        q("biblStruct"),
        {xml_attr("id"): SOURCE_WITNESS_ID},
    )
    ...
    text_element(
        project_desc,
        "p",
        "This TEI document is a derived machine-only view of Cora Histórico Digital. Internal CHD data remain canonical; OCR is preserved without silent correction or linguistic normalization.",
    )
    editorial_decl = ET.SubElement(encoding_desc, q("editorialDecl"))
    text_element(
        editorial_decl,
        "p",
        "Only machine_accepted alphabetical records are projected as lexical entries. machine_uncertain and machine_rejected candidates are retained as residual documentary items in back matter and are never promoted to entries.",
    )
    ...
    text_element(
        ...
        "language",
        "Spanish",
        ident=OBJECT_LANGUAGE,
        role="objectLanguage",
    )
    text_element(
        lang_usage,
        "language",
        "Cora / Náayeri (El Nayar Cora)",
        ident=TARGET_LANGUAGE,
        role="targetLanguage",
    )
    ...
    text_element(
        revision_desc,
        "change",
        f"Initial CHD projection targeting TEI Lex-0 {LEX0_VERSION}.",
        when="2026-09-11",
    )
...
def add_entry(body: ET.Element, record: dict[str, str]) -> None:
    ...
    entry = ET.SubElement(
        ...
        q("entry"),
        {
            ...
            xml_attr("lang"): OBJECT_LANGUAGE,
            "type": "mainEntry",
            "ana": "#machine_accepted #machine_derived #human_unverified",
            "source": f"#{SOURCE_WITNESS_ID}",
            "corresp": f"urn:chd:{candidate_id}",
            "n": record["source_printed_page"],
        },
    )
    ...
    sense = ET.SubElement(
        entry,
        q("sense"),
        {xml_attr("id"): f"{article_id}-s1"},
    )
    cit = ET.SubElement(
        sense,
        q("cit"),
        {"type": "translationEquivalent", xml_attr("lang"): TARGET_LANGUAGE},
    )
    translated_form = ET.SubElement(cit, q("form"), {"type": "lemma"})
    text_element(translated_form, "orth", record["cora_ocr"])

    provenance = (
        f"candidate_id={candidate_id}; pdf_page={record['source_pdf_page']}; "
        f"printed_page={record['source_printed_page']}; "
        f"ocr_lines={record['source_ocr_line_start']}-{record['source_ocr_line_end']}; "
        f"separator={record['separator_type']}; alignment={record['page_alignment_status']}; "
        f"confidence={record['extraction_confidence']}; rationale={record['machine_rationale']}"
    )
...
def add_residual(back: ET.Element, record: dict[str, str]) -> None:
    ...
    item = ET.SubElement(
        ...
        q("item"),
        {
            xml_attr("id"): record["candidate_id"],
            "ana": f"#{status} #machine_derived #human_unverified",
            "source": f"#{SOURCE_WITNESS_ID}",
            "n": record["source_printed_page"],
        },
    )
...
def build_tree(records: list[dict[str, str]]) -> tuple[ET.ElementTree, dict[str, object]]:
    ...
    report: dict[str, object] = {
        ...
        "projection_policy": (
            "machine_accepted records become TEI Lex-0 entries; machine_uncertain and "
            "machine_rejected records remain residual documentary items in back matter."
        ),
        "normalization_policy": (
            "headword_es_ocr and cora_ocr are emitted verbatim; no OCR correction, "
            "equivalent splitting, modern dialect assignment, or grammatical normalization occurs."
        ),
        "validation_scope": (
            "This subphase checks XML well-formedness, source coverage, ID stability and field fidelity. "
            "Full validation against the official TEI Lex-0 schema remains a separate gate before phase 4 closes."
        ),
    }
...
def main() -> None:
    ...
    print(
        f"TEI Lex-0 projection: {report['entry_total']} entries; "
        f"{report['residual_total']} residual candidates; target={LEX0_VERSION}"
    )
